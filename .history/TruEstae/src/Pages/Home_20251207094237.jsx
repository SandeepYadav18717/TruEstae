  ...
  <div style={{ margin: "20px", overflowX: "auto" }}>
    <table
      ...
      style={{
        ...
        background: "white",
        minWidth: "900px", // table never goes out from screen
      }}
    >
      <thead>
        <tr>
          {data.length > 0 &&
            Object.keys(data[0]).map((key) => (
              <th
                key={key}
                style={{ background: "#eee", textTransform: "capitalize", whiteSpace: "nowrap" }}
              >
                {key}
              </th>
            ))}
        ...
        {data.map((row, index) => (
          <tr key={index}>
            {Object.values(row).map((value, i) => (
              <td key={i} style={{ whiteSpace: "nowrap" }}>{value}</td>
            ))}
          </tr>
        ))}
      </tbody>
