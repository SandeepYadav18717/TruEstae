...
# ---------------- NAME + PHONE SEARCH API ----------------
@app.post("/search-name-phone")
def search_name_phone(data: SearchQuery):
    # Debug print
    print("SEARCH QUERY RECEIVED:", data.query)

    q = str(data.query).strip().lower()

    # Clean columns to avoid spaces, NaN, capital letters
...
def search_name_phone(data: SearchQuery):
    ...
    temp = df[
        df["Customer Name"].str.lower().str.contains(q)
        | df["Phone Number"].astype(str).str.contains(q)
    ]
